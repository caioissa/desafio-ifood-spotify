...
class SongRecommender():
    ...
    def recommend_playlist_by_cityname(self, cityname, length):
        temp = self.oi.get_temp_by_cityname(cityname)
        return self.get_playlist_by_temperature(temp, length)

    def recommend_playlist_by_latlon(self, lat, lon, lenght):
        temp = self.oi.get_temp_by_latlon(lat, lon)
        return self.get_playlist_by_temperature(temp, length)

    def get_playlist_by_temperature(self, temp, length):
